...
def lab_report_explanation(report: str, language: str = None) -> str:
    ...
    Supports Arabic ('ar') and English ('en') responses.

    Args:
        report: Plain text containing lab values.
        language: (Optional) The language to respond in ('ar' or 'en'). 
                 If provided, overrides automatic detection.
    """
    logger.info(f"[lab_tool] Received input: {report[:100]}... with language={language}")

    # ── Step 0: Detect language (if not provided) ──────────────────────────────
    if not language:
        language = detect_language(report)
        logger.info(f"[lab_tool] Auto-detected language: {language}")
    else:
        logger.info(f"[lab_tool] Using provided language: {language}")
